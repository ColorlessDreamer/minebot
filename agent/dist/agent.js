...
function trackPotentialAttacker(entity) {
    ...
    debugLog('entitySwingArm', `Tracking attacker: ${entity.username || entity.name || entity.type} (ID: ${entity.id})`, `Currently tracked: ${Array.from(state.potential_attackers.keys()).join(', ')}`);
    setTimeout(() => {
...
function initializeBotListeners() {
    ...
    bot.on('entitySwingArm', (entity) => {
        if (entity !== bot.entity) {
            ...
            debugLog('entitySwingArm', `Entity: ${entity.username || entity.name || entity.type} (ID: ${entity.id}) swinging arm`, `Distance to bot: ${distance.toFixed(2)}; Held item: ${heldItem ? heldItem.name : 'empty hand'}`);
    ...
    bot.on('entityMoved', (entity) => {
        ...
        if (distance <= DETECTION_RANGE && currentTime - lastMovementLog > MOVEMENT_LOG_INTERVAL) {
            debugLog('entityMoved', `Entity: ${entity.username || entity.name || entity.type} (ID: ${entity.id}) moved`, `Position: (${entity.position.x.toFixed(2)}, ${entity.position.y.toFixed(2)}, ${entity.position.z.toFixed(2)}); Distance to bot: ${distance.toFixed(2)} blocks`);
